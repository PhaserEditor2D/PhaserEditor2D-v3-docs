...
* Create a `Tilemap`_ object (and `TilemapLayer objects`_) in the |SceneEditor|_.

To create a `Tilemap`_ object, you can drag the **Tilemap** element from the `Blocks view <blocks-view-integration.html>`_ and drop it into the scene:
...
Because a `Tilemap`_ is not a display object, it is not shown in the scene, else in the |OutlineView|_, together with the tilesets:

.. image:: ../images/scene-editor-tilemap-outline-10152020.webp
    :alt: Tilemap object in the Outline view.

When you select a `Tilemap`_, the |InspectorView|_ shows some of its properties:
...
By default, a `Tilemap`_ is created with a class-scope. It means, a field is generated in code to reference the object, so you can manually configure other aspects of the map of create the layers:
