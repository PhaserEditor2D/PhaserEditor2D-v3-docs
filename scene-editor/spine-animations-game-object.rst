...
.. image:: ../images/spine-animations-add-game-object-1-20230924.webp
  :alt: Add spine game object to scene
...
* Finally, it adds the `SpineGameObject`_ to the scene:
  ...
  .. image:: ../images/spine-animations-add-obejct-done-20230924.webp
    :alt: Spine game object in scene.
...
``````````````````````````````````````````````````
If you imported the skeleton data and the atlas assets in the Asset Pack following the `default naming rules <spine-animations-loading.html#spine-assets-default-naming-rules>`_, then the |BlocksView|_ shows a especial **skin** block inside the skeleton data block.
