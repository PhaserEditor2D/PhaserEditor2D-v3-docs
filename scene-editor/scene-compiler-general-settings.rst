...
By the way, the name of the file and the class that is generated by the compiler, is the same name of the scene file. For example, a ``Zombie.scene`` file is compiled into ``Zombie.js`` (or ``Zombie.ts``) and the name of the class will be ``Zombie``.
