...
Look into the ``Level.js`` file, the |PrefabInstances|_ are created as expected, the trick here is that the code inside the **Doodle** constructor is your code, not the code generated by the |SceneCompiler|_. You only need to write a constructor signature that is compatible with the code of the ``Level.js`` file:
