...
The rule is simple. When you add a new file to the Asset Pack it builds the file's URL using the relative path to a root folder. To find the root folder, it looks for all the parent folders of the Asset Pack file. If it finds a parent folder that contains a ``publicroot`` file, then that folder is taken as the root folder to build the URL. If no ``publicroot`` file is found, then the root of the project is considered as the root folder to build the URL.
