...
For example, in the |AssetPackEditor|_, the user adds files to the |AssetPackFile|_. These files are shown as blocks in the |BlocksView|_, but when a user add a file to the editor, that file is hidden from the |BlocksView|_  because it is already present there.
...
The |BlocksView|_ is a `content viewer <../misc/content-viewers.html>`_: it shows inline preview of its elements, you can zoom in/out it and filter its content.
